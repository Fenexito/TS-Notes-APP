...
        let content = displayPrompt ? `<div class="popup-prompt">${displayPrompt}</div>` : '';
...
        switch (event.key) {
            case 'ArrowDown': event.preventDefault(); event.stopPropagation(); this.selectedIndex = (this.selectedIndex + 1) % this.items.length; this.updateSelected(); break;
            case 'ArrowUp': event.preventDefault(); event.stopPropagation(); this.selectedIndex = (this.selectedIndex - 1 + this.items.length) % this.items.length; this.updateSelected(); break;
            case 'Enter': case 'Tab': event.preventDefault(); event.stopPropagation(); this.onSelect(this.items[this.selectedIndex]); break;
            case 'Escape': event.preventDefault(); event.stopPropagation(); this.destroy(); break;
        }
    }
    
    _handleClick(event) {
...
                { key: 'cxtv', description: 'Flujo de TV.', steps: [ { id: 'issue', type: 'select', options: [ { label: 'stb no boot', value: 'tv is not powering on', nextStep: 'result' }, { label: 'recording', value: 'cx cannot record', nextStep: 'recordings' } ] }, { id: 'recordings', type: 'select', options: [ { label: 'rec list', value: 'cannot see the recording list', nextStep: 'result' }, { label: 'play rec', value: 'cx cannot play recordings', nextStep: 'result' } ] }, { id: 'result', type: 'template', template: '{issue} {recordings}.' } ] }
            ];
            this._saveShortcuts();
        }
    }
...
                element.addEventListener('click', () => { if (this._activePopupManager) this._activePopupManager.destroy(); });
            }
        }); 
    }
    
...
        if (this._activePopupManager) { this._activePopupManager.destroy(); this._activePopupManager = null; }
...
        if (triggerAt === -1) { this._insertTextAtCursor(replacementText); return; }
...
    const openModal = () => { modal.classList.add('visible'); renderShortcuts(); };
    const closeModal = () => { modal.classList.remove('visible'); };
    const showListView = () => { viewEditor.classList.add('hidden'); viewList.classList.remove('hidden'); renderShortcuts(); };
    const showEditorView = (shortcutKey = null) => { viewList.classList.add('hidden'); viewEditor.classList.remove('hidden'); currentEditingKey = shortcutKey; buildEditor(shortcutKey); };

    const setEditorMode = (mode) => {
        if (mode === 'dynamic') {
            simpleModeView.classList.add('hidden');
            dynamicModeView.classList.remove('hidden');
            addVarBtnContainer.textContent = "+ Añadir Variable";
        } else {
...
        const stepContainer = clone.querySelector('.step-container');
        const idInput = stepContainer.querySelector('.variable-id-input');
...
    };
    ...
    const updateTemplateWithVariable = (newVar, oldVar = '') => {
        ...
        let templateValue = templateTextarea.value;
        if (oldVar && templateValue.includes(oldVar)) {
            templateValue = templateValue.replace(oldVar, newVar);
        } else {
            templateValue += (templateValue ? ' ' : '') + newVar;
        }
        templateTextarea.value = templateValue;
        updateLivePreview();
    };
...
    };
    ...
    const updateOptionReorderButtons = (optionsContainer) => {
        ...
        options.forEach((option, index) => {
            option.querySelector('.move-option-up').disabled = (index === 0);
            option.querySelector('.move-option-down').disabled = (index === options.length - 1);
        });
    };

    const autoResizeTextareas = () => {
        document.querySelectorAll('textarea.step-textarea-flexible').forEach(textarea => {
            const updateHeight = () => {
...
        if (button.classList.contains('edit-btn')) { showEditorView(key); } 
        else if (button.classList.contains('move-up-btn')) { shortkeyManager.moveShortcut(key, 'up'); renderShortcuts(); } 
        else if (button.classList.contains('move-down-btn')) { shortkeyManager.moveShortcut(key, 'down'); renderShortcuts(); } 
